...
export async function POST(req: Request) {
  try {
    ...
    // append marker to indicate must-change on next login
    hashed = `${hashed}$mc`

    let { error } = await supabase.from('usuarios').update({ password: hashed, must_change_password: true }).eq('username', username)
    if (error) {
